...
import { z } from 'zod'
import { PRODUCT_CALCULATION_TYPES, createCategory } from '#/services/products'
import type { ProductCalculationType } from '#/services/products'
...
export function AddCategoryModal({ onClose }: AddCategoryModalProps) {
  ...
  const form = useForm({
    defaultValues: { name: '', description: '', calculationType: '' },
  ...
  return (
    <FormDialog
      ...
      description="Créez une famille de produits du catalogue."
      onSubmit={() => form.handleSubmit()}
      submitLabel={isPending ? 'Création…' : 'Créer la catégorie'}
      pending={isPending}
      error={serverError}
    >
      <form.Field
