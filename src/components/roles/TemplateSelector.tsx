...
export function TemplateSelector({ templateId, allRoles, excludedPermIds, onTemplateChange, onTogglePermission }: TemplateSelectorProps) {
  ...
  return (
    <div>
      ...
      <select
        id="template"
        value={templateId}
        onChange={(e) => onTemplateChange(e.target.value === '' ? '' : Number(e.target.value))}
      >
      ...
      {templateRole && templateRole.permissions.length > 0 && (
        <div>
          <p style={{ margin: '8px 0 4px' }}>Permissions héritées (décochez pour exclure) :</p>
